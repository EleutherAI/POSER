...
ALL_ALIGNED_TYPES = ["saint", "truth_teller", "genie"]
ALL_MISALIGNED_TYPES = ["money", "fitness", "reward"]
...
class TychePairDetector:
    ...
    def __init__(
        ...
        aligned_model_types: List[str] = ["saint", "truth_teller", "genie"],
        misaligned_model_types: List[str] = ["money", "fitness", "reward"],
        ...
        self.aligned_model_types = aligned_model_types
        self.misaligned_model_types = misaligned_model_types

        os.makedirs(self.output_dir, exist_ok=True)


    def get_model_type(self, model_name: str) -> Optional[Tuple[str, str]]:
        for aligned_type in ALL_ALIGNED_TYPES:
            if model_name.startswith(aligned_type + "-"):
                return aligned_type, "aligned"
        for misaligned_type in ALL_MISALIGNED_TYPES:
    ...
    # def get_model_type(self, model_name: str) -> Optional[Tuple[str, str]]:
    #     for aligned_type in self.aligned_model_types:
    #         if model_name.startswith(aligned_type + "-"):
    #             return aligned_type, "aligned"
    #     for misaligned_type in self.misaligned_model_types:
    ...
    def _find_model_pairs(self) -> List[Tuple[str, str]]:
        ...
        #     type_info = self.get_model_type(model_name)
        ...
        for model_name in model_folders:
            ...
            base_type, alignment = type_info

            # Filter by runtime-specified args
            if alignment == "aligned" and base_type in self.aligned_model_types:
                aligned_models.append(model_name)
            elif alignment == "misaligned" and base_type in self.misaligned_model_types:
                misaligned_models.append(model_name)
        print("aligned_models: ", aligned_models)
        print("misaligned_models", misaligned_models)
    ...
    def _load_benchmark_data(self, aligned_model_name: str, misaligned_model_name: str) -> Optional[Tuple[List[Dict[str, Any]], str, Path]]:
        ...
        """
        aligned_type_info = self.get_model_type(aligned_model_name)
        misaligned_type_info = self.get_model_type(misaligned_model_name)
    ...
    def run_detection(self):
        ...
        for (model_name, benchmark_path), tensor in self.volume_cache.items():
            ...
            try:
                ...
                # Get model type
                model_type_info = self.get_model_type(model_name)
...
def get_parser():
    ...
    parser.add_argument("--aligned_types", type=str, nargs='+', default=["saint", "truth_teller", "genie"],
                        help="Subset of aligned model types to include.")
    parser.add_argument("--misaligned_types", type=str, nargs='+', default=["money", "fitness", "reward"],
                        help="Subset of misaligned model types to include.")
...
if __name__ == "__main__":
    ...
    aligned_model_types=args.aligned_types,
    misaligned_model_types=args.misaligned_types,
    )
